...
/* Realiza o processamento de contextos para SQL e ArrayList.
 * 
 * Marcelo Barbosa,
 * junho, 2016. 
 */
...
class ProcessContext
{
        ...
        for($i = 0; $i < $context->getSize(); $i++)
        {
            ...
            if($contextElement->getIsKeyField())
            {
                ...
                if($i < ($context->getSize() - 1))
                {
                    ...
                    $nextContextElement = $context->getParameters()->get($i+1)->getObject();
    ...
    private static function getAmountOfNotKeyField(Context $context)
    {
        // retorna o número de elementos que não são chave
        $value = 0;
        
        for($i = 0; $i < $context->getSize(); $i++)
        {
            $contextElement = $context->getParameters()->get($i-1)->getObject();
    ...
    private static function getContextToArray(Context $context)
    {
        ...
        for($i = 0; $i < $context->getSize(); $i++)
        {
            ...
            if($contextElement->getValue() instanceof ArrayList)
            {
    ...
    public static function getListToArray(ArrayList $list)
    {        
    ...
    public static function getListToJSON(ArrayList $list)
    {        
        // retorna o conteudo de uma lista em um arquivo JSON
        // declaracao de variaveis
        $json = null;
    ...
    public static function getListToXML(ArrayList $list)
    {
